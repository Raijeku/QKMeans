...
def preprocess(points, map_type='angle', norm_relevance=False):
    ...
    if map_type == 'angle': 
        ...
        p_points = normalize(points[:])
        norms = (points/p_points)[:,0]
...
def batch_distance(B, backend, map_type='angle', shots=1024):
    ...
    if B[0].shape[1] == 2:
        if map_type == 'angle':
            ...
            return [(2-2*((1 - 2*M_i)**(1/2)))**(1/2) for M_i in M]
    elif B[0].shape[1] == 3:
        if map_type == 'angle':
            qcs = []
            for point in B[0]:
...
def batch_collect(batch_d, desired_shape):
    ...
    if batch_d.shape != desired_shape:
...
def batch_distances(X, cluster_centers, backend, map_type, shots, verbose):
    ...
    distance_list = np.asarray([batch_distance(B,backend,map_type,shots) for B in batches])
