...
def batch_distances(X, cluster_centers, backend, map_type, verbose):
    batches = batch_separate(X.to_numpy(), cluster_centers.to_numpy(),backend.configuration().max_experiments)
...
def qkmeans_plusplus(X, n_clusters, backend, map_type, verbose, x_squared_norms=None, n_local_trials=None, random_state=None):
    ...
    center_id = X.sample(n=1).reset_index(drop=True)
    ...
    for c in range(1, n_clusters):
        ...
        distance_to_candidates = batch_distances(X, X[candidate_ids], backend, map_type, verbose)
...
class QuantumKMeans():
    ...
    """
    def __init__(self, backend=IBMQ.load_account().get_backend('ibmq_qasm_simulator'), n_clusters=2, init='qk-means++', tol=0.0001, max_iter=300, verbose=False, map_type='angle', norm_relevance=False):
    ...
    def fit(self, X, batch=True):
        ...
        if self.verbose: print(self.init)
        if self.init == 'qk-means++': self.cluster_centers_, _ = qkmeans_plusplus(X, self.n_clusters, self.backend, self.map_type, self.verbose)
    ...
    def get_params(self, deep=True):
        ...
        """
        return {"n_clusters": self.n_clusters, "init": self.init, "tol": self.tol, "verbose": self.verbose, "max_iter": self.max_iter, "backend": self.backend, "map_type": self.map_type, "norm_relevance": self.norm_relevance }
